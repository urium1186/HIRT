...
namespace LibHIRT.Files
{
   ...
   */
    ...
    /// </summary>
    public abstract class HIRTStream : Stream
    {
    ...
    /// </summary>
    internal class HIRTExtractedFileStream : HIRTStream
    {
        ...
        private HIRTExtractedFileStream(Stream baseStream)
        {
            _baseStream = baseStream;
        }

        public static HIRTStream FromStream(Stream stream)
    ...
    internal sealed class HIRTDecompressionStream : HIRTStream
    {
        ...
        private const long HEADER_SIZE = 0x48;
...
#endif
            var bytesRead = _baseStream.Read(buffer, offset, size);
            _position += bytesRead;
            return bytesRead;
            /*
            if (size == 0)
    ...
    ///   A stream that reads from compressed/chunked files such as pck.
    /// </summary>
    internal sealed class HIRTDecompressionStreamOld : HIRTStream
    {
        ...
        private HIRTDecompressionStreamOld(Stream baseStream)
        {
            _baseStream = baseStream;
            _reader = new BinaryReader(_baseStream);
        }

        public static HIRTDecompressionStreamOld FromStream(Stream baseStream)
        {
            var stream = new HIRTDecompressionStreamOld(baseStream);
            stream.Initialize();

            return stream;
        }

        public static HIRTDecompressionStreamOld FromFile(string filePath)
          => HIRTDecompressionStreamOld.FromStream(File.OpenRead(filePath));
    ...
    internal sealed class HIRTStreamSegment : HIRTStream
    {

        #region Data Members

        private readonly HIRTStream _decompressionStream;
        ...
        public override long Position
        {
            get => _decompressionStream.Position;
            set
            {
                _decompressionStream.Position = value;
                /*var newPosition = _intValue;
        ...
        public HIRTStreamSegment(HIRTStream stream, long startOffset, long length)
        {
        ...
        {/*
            ...
            */
            _decompressionStream.Seek(offset, origin);
            //_position = offset - _startOffset;

            return _decompressionStream.Position;
        }
    ...
    /// </summary>
    internal sealed class HIRTStreamSegmentOld : HIRTStream
    {
        ...
        public HIRTStreamSegmentOld(HIRTStream stream, long startOffset, long length)
        {
            _decompressionStream = stream;
